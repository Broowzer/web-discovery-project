...
export default describeModule(
  ...
  () => {
    describe("ContentExtractor", function () {
      ...
      describe("#tryExtractBroowzerSerpQuery", function () {
        const expectNotFound = (url) => {
          if (uut.tryExtractBroowzerSerpQuery(url)) {
            chai.assert.fail(`Expected not to find a query on url=${url}`);
          }
        };

        it("should find search terms on search.broowzer.software", function () {
          expect(
            uut.tryExtractBroowzerSerpQuery(
              "https://search.broowzer.software/search?lang=en&country=us&safe_search=on&q=harzer%20k%C3%A4se"
            )
          ).to.equal("harzer käse");

          expect(
            uut.tryExtractBroowzerSerpQuery(
              "https://search.broowzer.software/search?q=m%C3%BCnchen&lang=en&country=de"
            )
          ).to.equal("münchen");
        });

        it("should find search terms on search.broowzer.com", function () {
          expect(
            uut.tryExtractBroowzerSerpQuery(
              "https://search.broowzer.com/search?lang=en&country=us&safe_search=on&q=harzer%20k%C3%A4se"
            )
          ).to.equal("harzer käse");

          expect(
            uut.tryExtractBroowzerSerpQuery(
              "https://search.broowzer.com/search?q=m%C3%BCnchen&lang=en&country=de"
            )
          ).to.equal("münchen");
        });

        it("should not find false positives", function () {
          ["https://search.broowzer.software/", "https://example.test/?q=test"].forEach(
            expectNotFound
          );
        });
